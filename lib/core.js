...
module.exports = {
    ...
    RunFromIndexBot: async function (index, modules) {
        var authToRun = [];
        for (let i = index; i < config.length; i++) {
            authToRun.push(config[i])
        }
        await this.DoRunBots(authToRun, modules);
    },
        ...
        if(settings.RunningMode.Mode == settings.Enums.RunningMode.All){
            ...
            for (let i = 0; i < accounts.length; i++) {
                ...
                if(buildSize >= size) {
                    ...
                    if(settings.RunningMode.DelayBetweenAccountModes != 0){
                        await this.awaitTime(settings.RunningMode.DelayBetweenAccountModes)
                    }
                    buildingcluster = [];
                    buildSize = 0;
                }
